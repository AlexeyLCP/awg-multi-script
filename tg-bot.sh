...
# ── Шаг 0: Проверка и удаление старой версии ──────────────────────────────────
echo ""
info "Проверяю наличие старой версии..."

OLD_FOUND=0
[[ -f "$BOT_PY" ]]      && OLD_FOUND=1
[[ -f "$BOT_SERVICE" ]] && OLD_FOUND=1
[[ -f "$BOT_CONF" ]]    && OLD_FOUND=1
systemctl list-unit-files awg-bot.service &>/dev/null && OLD_FOUND=1

if [[ $OLD_FOUND -eq 1 ]]; then
  warn "Найдена существующая установка бота"

  # Сохраняем токен и chat_id если есть — спросим переиспользовать ли
  OLD_TOKEN=""
  OLD_CHAT_ID=""
  if [[ -f "$BOT_CONF" ]]; then
  ...
  read -rp "$(echo -e "  ${Y}Удалить старую версию и установить новую? [y/N]: ${N}")" CONFIRM
  if [[ ! "$CONFIRM" =~ ^[Yy]$ ]]; then
    err "Установка отменена"
    exit 1
  fi

  info "Останавливаю и удаляю старую версию..."
  systemctl stop awg-bot 2>/dev/null    || true
  systemctl disable awg-bot 2>/dev/null || true
  rm -f "$BOT_SERVICE" "$BOT_PY"
  systemctl daemon-reload
  # Конфиг (с токеном) НЕ удаляем — переиспользуем
  # Лог НЕ трогаем — пусть остаётся история
  ok "Старая версия удалена"
else
  ok "Старая версия не найдена — чистая установка"
fi
...
# Подсказываем что есть сохранённый токен
if [[ -n "${OLD_TOKEN:-}" ]]; then
  TOKEN_PREVIEW="${OLD_TOKEN:0:10}...${OLD_TOKEN: -4}"
  info "Найден сохранённый токен: ${TOKEN_PREVIEW}"
  info "Нажми Enter чтобы использовать его, или вставь новый"
fi

read -rp "$(echo -e "  ${C}Вставь токен бота: ${N}")" BOT_TOKEN

# Если поле пустое и есть сохранённый — используем старый
if [[ -z "$BOT_TOKEN" && -n "${OLD_TOKEN:-}" ]]; then
  BOT_TOKEN="$OLD_TOKEN"
  info "Использую сохранённый токен из старой установки"
fi
...
if [[ -n "${OLD_CHAT_ID:-}" ]]; then
  info "Найден сохранённый chat_id: ${OLD_CHAT_ID}"
  info "Нажми Enter чтобы использовать его, или вставь новый"
fi

read -rp "$(echo -e "  ${C}Вставь свой chat_id: ${N}")" ADMIN_CHAT_ID

# Если пусто и есть старый — используем старый
if [[ -z "$ADMIN_CHAT_ID" && -n "${OLD_CHAT_ID:-}" ]]; then
  ADMIN_CHAT_ID="$OLD_CHAT_ID"
  info "Использую сохранённый chat_id"
fi
...
class Keyboards:
    ...
    @staticmethod
    def client_card(name: str) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup([
...
async def _cb_client_card(q, name: str, _ctx):
    ...
    await q.edit_message_text(
        ...
        reply_markup=kb.client_card(name)
    )
    ...
    try:
        ...
        if rc != 0:
            await q.edit_message_text(
                ...
                parse_mode=ParseMode.HTML, reply_markup=kb.client_card(name)
            )
            return
