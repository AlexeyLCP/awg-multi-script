...
[![Version](https://img.shields.io/badge/version-6.8.8--lucx-ff6b00?style=flat-square)](#)
...
```
╔══════════════════════════════════════════════╗
║    AWG Toolza: LucX Edition v6.8.8           ║
...
╚══════════════════════════════════════════════╝
  ...
  6)  Проверить домены из пулов (TCP + ping)
  ...
  ▸ Xray туннель [LucX]:
  18) Xray туннель  ● включен / ○ настроен, выключен / ○ не настроен

  ▸ Telegram-бот:
  19) Управление ботом  ● запущен / ○ выключен / ○ не установлен
...
## ☁ Warp туннель Cloudflare (пункт 15)
...
1. `15 → 8` — импорт готового профиля (например, через Google Cloud Shell)
...
## 🌉 Каскад — проброс на зарубежный VPS (пункт 17)
...
## 🔭 Xray туннель [LucX Edition] (пункт 18)
...
## 🤖 Telegram-бот (пункт 19)

Управление сервером со смартфона через inline-меню без SSH.

**Возможности:**
- Список клиентов: статус 🟢/🟡/🔴/⚫, трафик, последний handshake
...
Через главное меню: `sudo awg2` → **19) Управление ботом** → Установить.
...
- **Текст** — пункт 3 → 5 для больших конфигов (QUIC Full)
...
**AWG Toolza: LucX Edition v6.8.8** · Форк от [alexeylcp](https://github.com/AlexeyLCP)
