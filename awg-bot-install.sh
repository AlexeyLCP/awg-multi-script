...
# ── tg_api ────────────────────────────────────────────────────────────────────
# Вызов Telegram API без токена в argv (токен виден в `ps` на время запроса,
# если передавать URL аргументом). URL уходит через --config на stdin.
# Использование: tg_api METHOD [доп. аргументы curl...]
tg_api() {
  local method="$1"; shift
  printf 'url = "https://api.telegram.org/bot%s/%s"\n' "$BOT_TOKEN" "$method" | \
    curl -sf --max-time 15 --config - "$@" 2>/dev/null
}
...
LIVE_STATS_TTL = 10.0
CLIENTS_TTL    = 30.0
CLIENTS_PER_PAGE = 20   # пагинация списка: лимиты Telegram (4096 симв. текста,
                        # ~100 кнопок) ломали показ при ~78+ клиентах
...
class Keyboards:
    ...
    @staticmethod
    def clients(clients: list, stats: dict, page: int = 0) -> InlineKeyboardMarkup:
        # Пагинация: режем на страницы по CLIENTS_PER_PAGE, чтобы не упереться
        # в лимиты Telegram (макс ~100 кнопок и 4096 символов текста).
        total = len(clients)
        ...
        if pages > 1:
            nav = []
            if page > 0:
                nav.append(InlineKeyboardButton("◀️", callback_data=f"clients:{page-1}"))
            nav.append(InlineKeyboardButton(f"{page+1}/{pages}", callback_data="clients:noop"))
            if page < pages - 1:
                nav.append(InlineKeyboardButton("▶️", callback_data=f"clients:{page+1}"))
            rows.append(nav)
...
async def on_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── команды с данными (prefix:payload) ────────────────────────────────
        ...
        # Пагинация списка клиентов: "clients:2" / "clients:noop"
        if prefix == "clients":
            if payload != "noop":
                try:
                    await _cb_clients(q, int(payload))
                except (ValueError, TypeError):
                    await _cb_clients(q, 0)
            return
...
async def _cb_clients(q, page: int = 0):
    clients = get_clients()
    if not clients:
        ...
        return
    # Пагинация: показываем в тексте только клиентов текущей страницы,
    # иначе при 78+ клиентах текст превысит лимит Telegram (4096 символов).
    total = len(clients)
    pages = max(1, (total + CLIENTS_PER_PAGE - 1) // CLIENTS_PER_PAGE)
    page  = max(0, min(page, pages - 1))
    start = page * CLIENTS_PER_PAGE
...
async def on_add_name(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if srv_profile == "lite":
        ...
        auto_profile = "tls"       # Standard: I1 = TLS ClientHello (как в скрипте)
